// AI Churn prevention program
// Identify at-risk tenants, personalize retention offers
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const response = await fetch(baseUrl + '/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AISelfStorageFacilityManager'
    },
    body: JSON.stringify({
      model: MODEL,
      messages: [
...
router.post('/', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert AI assistant for AISelfStorageFacilityManager. Focus area: Churn prevention program. ${`Identify at-risk tenants, personalize retention offers`}. Respond ONLY with valid JSON (no markdown fences).`;
    const userPrompt = `Task: Churn prevention program.\n${`Identify at-risk tenants, personalize retention offers`}\n\nInput payload (JSON):\n${JSON.stringify(context, null, 2)}\n\nReturn JSON with the shape:\n{\n  "summary": "...",\n  "findings": ["..."],\n  "recommendations": ["..."],\n  "score": 0,\n  "confidence": 0\n}`;
    ...
    await persistResult(req.user?.id, 'churn-prevention', context, parsed);
    res.json({ feature: 'churn-prevention', model: MODEL, result: parsed });
  } catch (err) {
    console.error('[churn-prevention]', err.message);
...
router.get('/history', async (req, res) => {
  try {
    const r = await pool.query('SELECT id, endpoint, input_data, result, created_at FROM ai_results WHERE endpoint=$1 ORDER BY created_at DESC LIMIT 50', ['churn-prevention']);
